...
async function exportSeason(season) {
  ...
  const gamesRows = games.map(g => {
    ...
    return {
      ...
      sideA_p1_id:a1, sideA_p1_name:(playersById.get(a1)?.name||""),
      sideA_p2_id:a2, sideA_p2_name:(playersById.get(a2)?.name||""),
      sideB_p1_id:b1, sideB_p1_name:(playersById.get(b1)?.name||""),
      sideB_p2_id:b2, sideB_p2_name:(playersById.get(b2)?.name||""),
  ...
  for (const g of games) {
    ...
    for (const pid of [...g.sideA_player_ids, ...g.sideB_player_ids]) {
      ...
      pgsRows.push({
        ...
        player_id:pid, player_name:(playersById.get(pid)?.name||""), side,
        teammate_id, teammate_name:(playersById.get(teammate_id)?.name||""),
        opp1_id:opps[0], opp1_name:(playersById.get(opps[0])?.name||""),
        opp2_id:opps[1], opp2_name:(playersById.get(opps[1])?.name||""),
  ...
  for (const [pid,t] of totals.entries()) {
    ...
    pstRows.push({
      season_id:season.season_id, season_name:season.name, player_id:pid, player_name:(playersById.get(pid)?.name||""),
...
async function exportGame(game, season, playersById) {
  ...
  for (const pid of [...game.sideA_player_ids, ...game.sideB_player_ids]) {
    ...
    rows.push({
      ...
      sideA_p1:(playersById.get(a1)?.name||""), sideA_p2:(playersById.get(a2)?.name||""),
      sideB_p1:(playersById.get(b1)?.name||""), sideB_p2:(playersById.get(b2)?.name||""),
      final_score_a:scoreA, final_score_b:scoreB, winner_side,
      player_name:(playersById.get(pid)?.name||""), side, result,
      "2PM":line["2PM"], "2PMISS":line["2PMISS"], "2PA":d.twoPA, "2P_PCT":d.twoPct===null?"":d.twoPct.toFixed(4),
      "3PM":line["3PM"], "3PMISS":line["3PMISS"], "3PA":d.threePA, "3P_PCT":d.threePct===null?"":d.threePct.toFixed(4),
      "PTS":d.pts, "AST":line["AST"], "OREB":line["OREB"], "DREB":line["DREB"], "REB":d.reb, "STL":line["STL"], "BLK":line["BLK"]
    });
  ...
  const name = `${playersById.get(a1)?.name||"A1"}_${playersById.get(a2)?.name||"A2"}_vs_${playersById.get(b1)?.name||"B1"}_${playersById.get(b2)?.name||"B2"}`.replace(/\s+/g,'_');
