...
async function ensureCloudLoaded(){
  if (typeof sbHealth === "function") return true;
  // Try dynamically loading cloud script (in case cache/CDN hiccups prevented it)
  try{
    await new Promise((resolve,reject)=>{
      const s=document.createElement("script");
      s.src="supabase_client.v9-7.js";
      s.onload=()=>resolve();
      s.onerror=()=>reject(new Error("Failed to load cloud script"));
      document.head.appendChild(s);
    });
  }catch(e){}
  return (typeof sbHealth === "function");
}

// Stub to avoid hard crashes if cloud layer didn't load.
if (typeof sbHealth !== "function"){
  window.sbHealth = async ()=>({configured:false,signed_in:false,email:null,pending_ops:0,remaining:0});
}
...
async function init(){
  ...
  $("#btnSync").addEventListener("click", async()=>{
    if (!(await ensureCloudLoaded())) {
      showToast("Cloud not loaded. Refresh the page (Safari: pull down to refresh).");
      return;
    }
...
async function renderStart(app){
  ...
  for(const id of s.selected){
    const nm=(state.players.find(p=>p.player_id===id) && state.players.find(p=>p.player_id===id).name)||id;
...
async function renderLive(app){
  ...
  const name=(id)=>(playersById.get(id) && playersById.get(id).name)||"—";
...
function _pidName(pid, playersById){
  return (playersById.get(pid) && playersById.get(pid).name) || "—";
}
...
async function _computeElo(gamesChrono){
  ...
  for (const [pid, rating] of elo.entries()){
    const base = (start.get(pid)!=null ? start.get(pid) : 1000);
...
async function renderDraft(app){
  ...
  const getElo=(pid)=> (eloRes.elo.get(pid)!=null ? eloRes.elo.get(pid) : 1000);
